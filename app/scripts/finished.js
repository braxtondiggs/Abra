...
angular.module('abra.controllers')
	.controller('FinishedCtrl', ['$scope', '$rootScope', '$timeout', '$window', '$stateParams', '$cordovaDialogs', '$cordovaSocialSharing', '$cordovaAppAvailability', '$cordovaGoogleAnalytics', function($scope, $rootScope, $timeout, $window, $stateParams, $cordovaDialogs, $cordovaSocialSharing, $cordovaAppAvailability, $cordovaGoogleAnalytics) {
		...
		query.find({
  			success: function(results) {
  				...
  				for(var i = 0; i < results.length; i++) {
  					sum += parseInt(results[i].get('score'), 10);
  				}
  				$scope.average = Math.ceil(sum/results.length);
  				$scope.$apply();
  			}
  		});
		$timeout(function() {
